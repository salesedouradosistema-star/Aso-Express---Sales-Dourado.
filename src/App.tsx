...
import { Company, Doctor, ASORecord, SessionStats, JobRoleTemplate } from './types';
import {
  getActiveCompany,
...
export default function App() {
  ...
  // Monitor Firebase Google Auth and Whitelist Permissions
  useEffect(() => {
    ...
    const unsubAuth = onAuthChange(async (user) => {
      ...
      if (isMasterAdminEmail(user.email)) {
        ...
        // Check whitelist
        try {
          const authorized = await checkIfEmailIsAuthorized(user.email);
      ...
      unsubWhitelist = subscribeToAuthorizedUsers((list) => {
        ...
        const allowed =
          ...
          list.some((item) => item.email.toLowerCase().trim() === cleanEmail);
  ...
  const handleQuickDemoFill = () => {
    ...
    const random = demoEmployees[Math.floor(Math.random() * demoEmployees.length)];
    
    // Dispatch custom event or set asoToEdit dummy
    const demoASO: ASORecord = {
      id: `demo-${Date.now()}`,
      asoCode: `ASO-2026-${Math.floor(1000 + Math.random() * 9000)}`,
      ...
      employee: {
        ...random,
        age: new Date().getFullYear() - parseInt(random.birthDate.slice(0, 4)),
      },
      examType: 'em_branco',
      risks: {
        ...DEFAULT_RISKS,
      ...
      complementaryExams: [
        { id: 'c-1', name: 'Avaliação Clínica Ocupacional', date: new Date().toISOString().slice(0, 10), result: 'normal' },
        { id: 'c-2', name: 'Audiometria Tonal Ocupacional', date: new Date().toISOString().slice(0, 10), result: 'normal' },
      ],
      questionnaire: {
        enabled: true,
        categories: [
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans transition-colors duration-200">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {activeTab === 'attendance' && (
          <AttendanceFlow
            ...
            onRoleChangeInFlow={(id) => setSelectedRoleId(id)}
      ...
      {/* Access Management Modal (Exclusive to Master Admin) */}
